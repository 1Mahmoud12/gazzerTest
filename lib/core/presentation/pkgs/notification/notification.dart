  ...
  static void onTapNotificationScreenNavigateCallback(
    ...
    if (notificationType.toLowerCase() == NotificationType.support.name) {
      ...
      AppNavigator.mainKey.currentState?.push(
        MaterialPageRoute(
          builder: (context) => GazzerSupportChatScreen(
            chatId: chatId,
            orderId: orderId,
          ),
        ),
...
Future<Map> loadJsonFile() async {
  final String response = await rootBundle.loadString(
    'assets/services/gazar-21127-firebase-adminsdk-j5y44-5ac7ce9b3e.json',
  );
...
Future<void> sendFCMMessage({
  ...
  final response = await authClient.post(
    Uri.parse(
      'https://fcm.googleapis.com/v1/projects/codgoo-12e2d/messages:send',
    ),
    headers: {
