...
class _FavoriteWidgetState extends State<FavoriteWidget> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      isFav.value = bus.isFavorite(widget.fovorable);
    });
  ...
  void dispose() {
    ...
    bus.unSubscribe<ClearFavorites>(clearListener);
    lisnter.cancel();
    clearListener.cancel();
    controller.dispose();
    super.dispose();
  }
...
class DecoratedFavoriteWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    //       ),
    //       child: FavoriteWidget(key: ValueKey(fovorable.id), size: size, padding: padding, fovorable: fovorable, color: Co.bg),
    //     ),
    //   );
    // }
    // return DecoratedBox(
