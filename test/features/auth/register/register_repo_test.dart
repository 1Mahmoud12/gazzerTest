...
void main() async {
  ...
  group('RegisterRepo Tests', () {
    ...
    group('Register Function Tests', () {
      test('should return AuthResponse with sessionId and message when registration succeeds', () async {
        when(
      ...
      test('should return Error with message when registration fails', () async {
        when(apiClient.post(endpoint: Endpoints.register, requestBody: registerData.registerReq.toJson())).thenThrow(() {
    ...
    group('ResendOTP Function Tests', () {
      test('should return success message when resend OTP succeeds', () async {
        when(
      ...
      test('should return error message when resend OTP fails', () async {
        when(apiClient.post(endpoint: Endpoints.resendOtp, requestBody: {'session_id': registerData.sessionId})).thenThrow(() {
    ...
    group('EditPhoneNumber Function Tests', () {
      test('should return success message when phone number edit succeeds', () async {
        when(
          apiClient.post(endpoint: Endpoints.editPhoneNum, requestBody: {'phone': registerData.newPhone, 'session_id': registerData.sessionId}),
        ).thenAnswer((_) async => successResponse..data = registerData.editPhoneNumberSuccessJson);
      ...
      test('should return Error when phone number edit fails due to session not existing', () async {
        when(
          apiClient.post(
      ...
      test('should return Error when phone number edit fails due to phone already taken', () async {
        when(
          apiClient.post(endpoint: Endpoints.editPhoneNum, requestBody: {'phone': registerData.existPhoneNum, 'session_id': registerData.sessionId}),
        ).thenThrow(() {
    ...
    group('VerifyOTP Function Tests', () {
      test('should return success message when OTP verification succeeds', () async {
        when(
          apiClient.post(endpoint: Endpoints.verifyOTP, requestBody: {'session_id': registerData.sessionId, 'otp_code': registerData.code}),
      ...
      test('should return Error when OTP verification fails', () async {
        when(
          apiClient.post(endpoint: Endpoints.verifyOTP, requestBody: {'session_id': registerData.sessionId, 'otp_code': registerData.code}),
        ).thenThrow(() {
    ...
    group('Verify Repo Function Tests', () {
      group('Verify Function Tests', () {
        ...
        test('should return Error when OTP verification fails', () async {
          when(
            apiClient.post(endpoint: Endpoints.verifyOTP, requestBody: {'session_id': registerData.sessionId, 'otp_code': registerData.invalidCode}),
          ).thenThrow(() {
            errorResponse.response!.data = registerData.verifyOtpErrorJson;
            return errorResponse;
          }());
